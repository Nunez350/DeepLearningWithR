...
for (gene in 1:length(weight_bias_list)){
for (generation in 1:1000){
    ...
    L<-weight_bias_list[[3]]$training.snps %*%weight_bias_list[[3]]$weight_bias
    ...
    round(y)
    which(round(y[,1])==weight_bias_list[[1]]$targets[,1] & round(y[,2])==weight_bias_list[[1]]$targets[,2] & round(y[,3])==weight_bias_list[[1]]$targets[,3])
